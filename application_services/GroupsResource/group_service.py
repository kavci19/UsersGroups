...
class GroupResource(BaseApplicationResource):
    ...
    def insert_by_template(cls, template):
        res = DBService.insert_group_by_template("UsersGroups", "Groups", 'group_id', template)
        return res

    @classmethod
    def delete_by_id(cls, id_to_delete):
        DBService.delete_by_id("UsersGroups", "BelongsTo", "group_id", id_to_delete)
        res = DBService.delete_by_id("UsersGroups", "Groups", "group_id", id_to_delete)
        return res

    @classmethod
    def get_by_id(cls, id_to_get):
        res = DBService.get_by_id("UsersGroups", "Groups", "group_id", id_to_get)
        return res

    @classmethod
    def update_by_id(cls, template, id_no):
        res = DBService.update_by_id('UsersGroups', 'Groups', template, 'group_id', id_no)
        return res
    ...
    def remove_user_from_group(cls, template, group_id):
        username = template["username"]
        res = DBService.remove_user_from_group("UsersGroups", "BelongsTo", group_id, username)
        return res
